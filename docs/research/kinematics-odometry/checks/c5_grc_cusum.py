"""GRC final variant: no per-update clip, 3-sigma innovation gate, P eigen-clip, physical bounds,
...
class GRC:
    def __init__(s, psi0=None):
        ...
        s.gp = s.gm = 0.0; s.armed = False; s.frozen = False; s.t_alarm = None
    def step(s, h, z, R, t):
        Pm = s.P + q*np.eye(2)
        nu = z - h@s.psi; S = h@Pm@h + R; nn = nu/np.sqrt(S)
        if not s.armed and np.linalg.eigvalsh(Pm).max() <= Parm: s.armed = True
        ...
        if not s.frozen and nu**2 <= 9*S:
            K = Pm@h/S; s.psi = np.clip(s.psi + K*nu, 0.95*psi_nom, 1.05*psi_nom); Pm = (np.eye(2)-np.outer(K, h))@Pm
...
def drive(g, psi_true, segs, rg, t0=0.0, slipL=0.0, track=None):
    ...
    for dur, v, w in segs:
        for _ in range(int(round(dur/0.1))):
            ...
            if abs(hR) > 1.0 and abs(hL) > 1.0: g.step(np.array([hR, -hL]), z, R, t)
            t += 0.1
