...
Variants
  ...
  pvt3s : brief-v1-like 3 s probabilistic TTC/risk costs (soft radius)  -> ablation A6
  blind : dynamic obstacles ignored (sanity check that scenarios are actually conflicting)
Run history: run1 = measured-velocity window centre, run2 = arc extension + 6 m pass trigger + soft-radius risk (w_r=2),
run3 = path-parallel extension + closing-time trigger + hard-radius risk (w_r=1), T_ACT 2.5,
run4 = run3 + T_ACT 3.0 (brief value) + strict 1 s hold window in the return-time metric.
...
DELTA_HARD, DELTA_TTC, T_ACT, T_RISK = 0.05, 0.05, 3.0, 1.5   # run4: T_ACT 2.5 -> 3.0 (= brief T_act = tau_warn)
...
PASS_TRIGGER = "time"                             # "time": closing time along path < T_PRED; "dist6": first draft (ahead < 6 m)
RES = 0.05
...
class Planner:
    ...
    def plan(self, st, tracks, world, v_cap, x_goal, tnow):
        ...
        for tr in tracks:
            ...
            trig = (ahead / close < T_PRED) if PASS_TRIGGER == "time" else (ahead < 6.0)
            if -(R + 0.5) < ahead and trig and abs(tr["p"][1]) < R + 0.3 and abs(tr["u"][1]) < 0.3 \
...
def run(args):
    ...
    ret = None; t_clear = None; t_ret = None; ret_trunc = False
    if t_start is not None:
        ...
        if t_clear is not None:
            ...
            for i in idx:
                j2 = i + 100
                if ok[i:min(j2, len(ok))].all(): t_ret = tt[i]; ret_trunc = j2 > len(ok); break   # run4: strict hold window
    ...
    return dict(sid=sid, seed=seed, variant=variant, collided=bool(collided), min_edge=round(float(min_edge), 3),
                ...
                t_start=None if t_start is None else round(float(t_start), 2), t_clear=None if t_clear is None else round(float(t_clear), 2),
                ret_trunc=bool(ret_trunc),
