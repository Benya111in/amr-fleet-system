...
- **포켓 선택**: `traffic_zones.yaml` 의 포켓 중 y 에서 **다른 로봇 몸체(반경 2r)와 다른 로봇이 보유 · 점유한 구역을
  ...
  셀을 쓰고, **구역 밖에도 자리가 없으면 구역 안까지 허용한다**(`pocket_into_zone`, 마지막 수단) — 1차선 통로·
  막다른 접근로는 폭이 `pocket_clearance_m` 보다 좁아 '구역 밖' 후보가 전부 걸러지기 때문이다 (통합 시나리오 12
  실측: 해소 실패 5건이 모두 `no_pocket,no_route`). 구역 안이라도 다른 로봇의 남은 경로에서는 같은 간격을
  지키므로 상대는 지나갈 수 있고, 그 사실은 사건 기록에 `YIELD:<robot>(into_zone)` 으로 남는다. 통과 격자는 "셀 안 한 점이라도 벽까지 ≥ 0.22 m" 로 만들어 폭 0.6 m 좁은 통로를 지나는 길을 보존한다.
