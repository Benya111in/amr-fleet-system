...
모든 값은 YAML 로 외부화되어 있고 `cost_weight`, `tolerance`, `max_planning_time`, `smoother.w_*`,
`enable_*` 는 실행 중 `ros2 param set /amr_01/planner_server AStar.cost_weight 3.0` 으로 바꿀 수 있다
(재빌드·재시작 불필요, 명세 9장).
...
- **C3 속도 프로파일·시간 예측**: `core::SpeedProfile::predictTravelTime` (곡률 상한 + 가감속 원뿔 + S-curve 보정),
  ...
  ([path_tracking.md](path_tracking.md) §5: 폐루프 12 목표 평균 2.2–5.0 %, 최대 8.1 %).

## 7. 벤치마크 방법

- **지도**: `amr_navigation/warehouse_map.py` 합성 창고 60 × 40 m @ 0.05 m = **1200 × 800** (Gazebo 월드
  `gen_warehouse_world.py` 의 정적 배치 — 외벽, 랙 3 열 × 7, 좁은 통로 0.60 m, 기둥 8, 충전기, 도크 화물).
- **쌍**: `make_warehouse_map.py --pairs 50 --seed 42` — 두 점 모두 여유거리 ≥ 0.45 m(외접 반경 + 여유), 직선거리 ≥ 5 m.
- **코어 측정** (`astar_benchmark`, ROS 없음): Nav2 식 팽창(0.2 / 1.2 / 2.0) 후 A* + 평활화, 쌍마다 3 회 반복 중
  최솟값(외부 부하 잡음 제거).
- **Nav2 비교** (`planner_benchmark.launch.py` + `bench_planners.py`): 같은 지도·쌍으로 `planner_server` 에서
  AStar / NavFn / Smac 를 `compute_path_to_pose` (use_start) 로 호출, 서버 측 `planning_time` 과 왕복 시간,
  경로 길이, 최소 여유거리(지도 EDT, 로봇 중심)를 기록.

## 8. 결과

측정 환경: `amr-fleet-system:wf-final` 컨테이너, 호스트 32 스레드, **load average 2.2–2.4** (`uptime`, 외부 부하
종료 후 — 비잠정 측정). 재현: `make_warehouse_map.py --pairs 50 --seed 42` → `astar_benchmark` →
`planner_benchmark.launch.py` + `bench_planners.py` (원자료 `bench_planners.csv`, `astar_core.csv`).

### 8.1 코어 A* (ROS 없음, 1200 × 800, 50 쌍, 쌍마다 3 회 중 최솟값)

| 지표 | 평균 | p95 | 최대 |
| --- | --- | --- | --- |
| 성공 | 50/50 (100 %) | | |
| 탐색 시간 | 3.71 ms | 15.19 ms | 18.93 ms |
| 평활화 시간 | 1.7 ms | | 8.6 ms |
| 합계 (A* + 평활화) | 5.42 ms | 18.88 ms | **22.22 ms** (명세 500 ms 의 4.4 %) |
| 확장 셀 수 | 25 435 | | 119 870 (전체 96 만 셀의 12.5 %) |
| 경로 길이 (격자 → 평활) | 31.15 → 30.26 m (−2.8 %) | | |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| **AStar (본 구현)** | **50/50 (100 %)** | **5.6 / 18.5 / 22.9** | **14.8** | 30.27 | 1.000 | 1.214 / 0.461 | 1.296 / 0.750 |
| NavFn (Dijkstra, 보간) | 50/50 (100 %) | 10.0 / 16.4 / 19.2 | 29.4 | 29.98 | 0.993 | 1.257 / 0.461 | 1.404 / 1.140 |
| Smac 2D | 50/50 (100 %) | 23.1 / 79.1 / 118.1 | 32.6 | 30.49 | 1.007 | 1.264 / 0.461 | 1.387 / 0.814 |

- **명세 판정**: 성공률 100 % (목표 ≥ 98 %), 재계획(서버 계획 시간) 최대 22.9 ms · 왕복(요청 → 경로 수신) 평균
  14.8 ms (목표 < 500 ms). 외부 부하(load 72–97) 아래 같은 벤치마크(잠정)에서도 AStar 최대 60.9 ms, Smac 최대
  275.6 ms 로 목표 안이었다.
- **길이**: 세 계획기 차이 ±0.7 % — 같은 비용 가중(κ = 2)이라 같은 통로를 고른다. NavFn 이 가장 짧은 것은 격자
  경로가 아니라 퍼텐셜 기울기를 따라 보간하기 때문이다.
- **여유거리**: 끝점(시작·목표를 여유 ≥ 0.45 m 로 뽑음)을 뺀 경로 중간 최소 여유는 AStar 0.75 m 로 NavFn(1.14)보다
  작다. 추정 원인: 비용 인지 숏컷이 "원래 격자 경로의 최대 비용 + 10" 까지 허용하므로 랙 모서리를 돌 때 중심선이 벽 쪽으로
  당겨진다 (길이 −2.8 % 와 교환, 미검증 — 쌍별 격자 경로 여유와의 비교는 하지 않았다). 0.75 m 는 외접 반경 0.361 m 의 2 배이므로 충돌 여유는 충분하지만, 더 넓은
  여유가 필요하면 `smoother.clearance_cost_margin` 을 10 → 0, `w_clearance` 를 0.3 → 0.6 으로 올린다 (§5).
- **계획 시간**: AStar 가 평균 1.8 배(NavFn), 4.1 배(Smac) 빠르다 — 옥타일 휴리스틱(허용·일관), 세대 번호 방문 표시,
  1/1024 셀 동점 양자화(§3). Smac 2D 는 자체 평활기·방향 계산을 포함해 p95 가 길다.

### 8.3 좁은 통로 (costmap.md §3)

무작위 50 쌍 중 0.60 m 통로를 지나는 쌍은 없었다 (우회 ≤ 6.5 m 면 넓은 통로 선호, costmap.md §4.1). 통로 관통은 폐루프
시험에서 따로 확인했다: `closed_loop_eval.py` 의 `narrow` 경로 (0, −6) → (0, −14.5) 를 DWA·Pure Pursuit 모두 1.0 m/s
로 통과, 풋프린트 최소 여유 **0.10 m** (= 기하 여유 0.30 − 0.20), 충돌 0 ([dwa.md](dwa.md) §8, [path_tracking.md](path_tracking.md) §5).
전역 계획기가 통로 안 목표를 계획하는 경우는 단위 테스트 `AStarPlannerPlugin.PlansThroughAisleAndHandlesErrors`
(1.2 m 통로 중앙 ±0.15 m)와 `test_warehouse_map.py` 로 확인한다.
