...
def test_footprints_and_agreement(world):
    shapes = worldmap.footprints(world / 'w.sdf', world / 'models', 0.38)
...
def test_evaluation_analyze(tmp_path):
    ...
    with open(tmp_path / 'response_time.csv', 'w') as fh:
        fh.write(','.join(metrics.RESPONSE_COLUMNS) + '\n')
        fh.writelines(f'{i},{i + 0.1},100.0,t{i}\n' for i in range(50))
    ...
    assert (tmp_path / 'report.md').is_file() and result['passed']
